...
def add_file(name_data, filename):
    ...
    with open(filename) as f:
        for line in f:
            ...
            if len(line_info) == 1:
                year = line_info[0].strip()

            if len(line_info) == 3:
                rank = line_info[0].strip()
                name1 = line_info[1].strip()
                name2 = line_info[2].strip()
...
def search_names(name_data, target):
    ...
    """
    matching_names = []
    for names, value in name_data.items():
        if names.lower().find(target) != -1:
            matching_names.append(names)
